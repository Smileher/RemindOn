...
src-tauri/target/release/bundle/nsis/RemindOn_0.5.0_x64-setup.exe
```

当前项目的便携版 ZIP 是由 release 单文件程序压缩得到的。构建完成后，可以在 PowerShell 中执行：

```powershell
$zip = 'src-tauri/target/release/bundle/RemindOn_0.5.0_x64_portable.zip'
...
“启动后自动最小化”只决定程序启动时是否直接隐藏到托盘。无论是否启用，点击主窗口关闭按钮都会隐藏到托盘并继续提醒；需要完全结束程序时，请使用托盘菜单中的“退出”。
...
- `next_recurring()`：计算每天、每周和每月的下一次时间。
...
界面中的通知方式可以设置为 `system`（系统通知）或 `popup`（软件通知），默认为 `popup`。系统通知仅显示简洁提示；软件通知是独立的带标题栏窗口，居中显示，支持完成和稍后提醒，可选简洁、标准或醒目样式。稍后提醒可选 30 秒、1 分钟、5 分钟、10 分钟、30 分钟或 1 至 4 小时。

事件提醒、休息提醒和定时操作页面的标题右侧分别提供“测试通知”。事件提醒测试遵循当前通知方式和样式；休息提醒测试使用当前休息文案并展示独立的已休息计时，不会暂停或重置真实休息倒计时；定时操作测试始终显示带 60 秒倒计时的软件通知，在倒计时结束或点击立即执行时还会要求原生确认，取消确认不会执行系统操作。真实定时操作到期后仍按既有规则直接执行，不显示测试确认框。

休息提醒弹出软件通知后会暂停下一轮计时，主窗口显示“正在休息”。关闭通知代表本轮休息完成，此时才从完整间隔重新计时；使用稍后提醒则按所选时长重新安排。系统通知无法感知用户何时关闭，因此发出后会直接开始下一轮计时。
...
系统通知由 Rust 后端直接调用 Tauri 通知插件，并使用 RemindOn 的应用标识，不通过 PowerShell 脚本发送。软件通知由隐藏的 `reminder` 窗口显示；后端会先居中、恢复最小化状态、置顶并显示窗口，再定向发送提醒内容。
...
先确认运行的是刚构建的 `0.5`（构建元数据为 `0.5.0`），并完全退出托盘中的旧实例。安装版、便携版和开发版共享上面列出的标准配置文件；如果同时保留不同开发版本，旧程序可能用旧数据结构重写同一个文件。新版保存失败时会在页面显示原因，不会只改变界面而不提示。
