...
界面中的通知方式可以设置为 `system`（系统通知）或 `popup`（软件通知），默认为 `popup`。系统通知仅显示简洁提示；软件通知是独立的带标题栏窗口，居中显示，支持完成和稍后提醒，可选简洁、标准或醒目样式。稍后提醒可选 30 秒、1 分钟、5 分钟、10 分钟、30 分钟或 1 至 4 小时。参数设置页的“测试通知”会先保存当前选项，再使用当前通知方式和样式发送测试。
