...
const names = ['RemindOn_0.6.0_x64-setup.exe', 'RemindOn_aarch64.app.tar.gz', 'RemindOn_x64.app.tar.gz']
const release = {
  tag_name: 'v0.6.0',
  body: '更新说明',
...
test('one manifest contains the signed Windows and both macOS packages', () => {
  ...
  assert.equal(manifest.platforms['darwin-x86_64'].url, release.assets[2].browser_download_url)
...
test('a missing platform, missing signature or mismatched version stops publication', () => {
  ...
  assert.throws(() => createUpdaterManifest('0.6.0', release, { ...signatures, [`${names[2]}.sig`]: '  ' }), /darwin-x86_64/)
})
