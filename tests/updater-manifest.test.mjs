...
const names = [
  'RemindOn_0.6.0_x64-setup.exe',
  'RemindOn_aarch64.app.tar.gz',
  'RemindOn_0.6.0_x64_portable.exe',
  'RemindOn_0.6.0_aarch64.dmg',
]
const release = {
  tag_name: 'v0.6.0',
  body: '更新说明',
...
test('one manifest contains the signed Windows and Apple Silicon packages', () => {
  const manifest = createUpdaterManifest('0.6.0', release, signatures, checksums)
...
test('a missing platform, missing signature or mismatched version stops publication', () => {
  assert.throws(() => createUpdaterManifest('0.7.0', release, signatures, checksums), /version/)
  assert.throws(() => createUpdaterManifest('0.6.0', { ...release, assets: release.assets.slice(1) }, signatures, checksums), /windows-x86_64/)
  assert.throws(() => createUpdaterManifest('0.6.0', release, { ...signatures, [`${names[1]}.sig`]: '  ' }, checksums), /darwin-aarch64/)
  assert.throws(() => createUpdaterManifest('0.6.0', release, signatures, { ...checksums, [`${names[2]}.sha256`]: 'invalid' }), /windows-x86_64-portable/)
})
...
test('draft asset URLs are converted to permanent version URLs before publication', () => {
  ...
  const manifest = createUpdaterManifest('0.6.0', draft, signatures, checksums)
